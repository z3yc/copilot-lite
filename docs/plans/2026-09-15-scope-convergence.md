...
| I | 企业化:租户与权限 | ❄️ 冻结(§11) |
| J | 可靠性与治理 | 🟡 J1/J3 在范围内(J3 → R5);J2/J4/J6/J7 ❄️ 冻结(J5 并入 L) |
...
```
Expected: 仅剩 **21 条**：R1–R7（7）+ §9.5/§10.5/§14 验收门槛（5+6+1=12）+ §12 推送（**2**）；**不应再出现 I/K/M2-M6/N2.7/Q2-Q4/Z 批次的功能条目**。

> 注意：本步只看「复选框是否有归属」，不看数量是否变小。经过 **Task 5b** 与 Step 4 后，本文件未勾选项最终为 **20**。

**Step 4: 修正 §14 陈旧条目(批次 L 已完成)**

`## 14. 验收总门槛` 末行 `- [ ] 删除操作全量软删除(...)(批次 L)` 与 §4「L ✅ 完成」矛盾,改为:
...
```bash
git add OPTIMIZATION_PLAN.md
git commit -m "docs(plan): 路线图重写为 R0-R7 并清理重复与陈旧记录

- §13 由 11 项建议顺序改为 R0-R7 交付路线,含预估与验收镜头对应
- 明确 R1-R7 实施计划不超前写(R4 需先定 MCP server 选型,R6 依赖 R5 执行器)
- §12 删除重复的推送条目;§14 修正批次 L 已完成的陈旧未勾选
- 保留 D3 并入 G-M2 / J5 并入 L 的溯源注记(防止批次沿革信息丢失)"
```

---

### Task 5b: 批次章节去复选框（冻结/在范围项归一）

> **来源**：预检遗漏，由 Batch A 实现者发现（见 `.superpowers/sdd/2026-09-15-scope-convergence/batch-a-report.md`）。
> **问题**：`§3`/`§5`–`§10` 的功能条目仍以 `- [ ]` 存在，与 §11 冻结清单、§13 路线图**重复计数**——冻结项看起来仍像待办，未完成项虚高（实测 51 而非预期值）。
> **目标**：每个交付物**只在 §13 有一处进度复选框**；批次章节降为描述性文字。

**Files:**
- Modify: `OPTIMIZATION_PLAN.md`（§3、§5、§6、§7、§8、§9.2、§10.2）

**Step 1: 冻结项去复选框**

把以下章节里**冻结项**的 `- [ ] ` 前缀改为 `- ❄️ `（文字不变）：

| 章节 | 条目 |
|---|---|
| §3 | `G-M4`（1 条） |
| §5 | `I1`–`I6`（6 条） |
| §6 | `J2`、`J4`、`J6`、`J7`（4 条；`J3` 见 Step 2；`J5` 已是说明行） |
| §7 | `K1`–`K5`（5 条） |
| §8 | `M2`、`M3`、`M4`、`M6`（4 条；`M5` 见 Step 2） |
| §10.2 | `Q2`、`Q3`、`Q4`（3 条；`Q1`/`Q5` 见 Step 2） |

**Step 2: 在范围项去复选框并指向批次**

把**在范围项**的 `- [ ] ` 前缀改为 `- 🟡 `，并在**行尾**追加 `（见 §13 R<N>）`：
...
| §8 | `M5` | `（见 §13 R3）` |
| §9.2 | `P-F1` | `（见 §13 R4）` |
| §9.2 | `P-F2`、`P-F3`、`P-F4` | `（见 §13 R7）` |
| §10.2 | `Q1` | `（见 §13 R2）` |
| §10.2 | `Q5` | `（见 §13 R5）` |

**Step 3: 不动门槛清单**

`§9.5` 与 `§10.5` 的验收门槛复选框**保持 `- [ ]` 不变**——它们是批次验收清单，不是待办功能项。

**Step 4: 校验**

```bash
grep -c "^- \[ \]" OPTIMIZATION_PLAN.md
grep -c "^- ❄️" OPTIMIZATION_PLAN.md
grep -c "^- 🟡" OPTIMIZATION_PLAN.md
```
Expected: **20** / **23** / **8**

- **20** = §13 R1–R7（7）+ §9.5（5）+ §10.5（6）+ §12 推送（2）
- **23** = §3（1）+ §5（6）+ §6 的 J2/J4/J6/J7（4）+ §7（5）+ §8 的 M2/M3/M4/M6（4）+ §10.2 的 Q2/Q3/Q4（3）
- **8** = §6 J3（1）+ §8 M5（1）+ §9.2 P-F1–P-F4（4）+ §10.2 Q1/Q5（2）

三个计数任何一个不符都说明改多了或改少了，逐条核对后再提交。

**Step 5: 提交**

```bash
git add OPTIMIZATION_PLAN.md
git commit -m "docs(plan): 批次章节去复选框——冻结项与在范围项归一，消除重复计数

- 冻结项改 ❄️、在范围项改 🟡 并指向 §13 对应批次，取消 - [ ] 待办语义
- 每个交付物只在 §13 保留一处进度复选框（未完成项 51 → 20）
- 修复冻结项仍表现为待办、导致'无归属'判据失效的缺陷"
```
...
```
Expected: 约 **78**(与基线 77 近乎持平--R0 新增了 R1-R7 与 P6 复选框,同时删掉了 §11 暂缓批次与重复推送)。

> ⚠️ **判定不看总数**(总数会因新增跟踪复选框而持平甚至上升),只看下面两个指标。
...
| 类别 | 预期条数 | 说明 |
|---|---|---|
| 功能/交付项 | ≤ 30（预期 27） | OPTIMIZATION_PLAN 7（R1–R7）+ ADMIN_PLAN 7（N2.1–N2.6、N2.8）+ PLAN 13（P5 6 + P6 7） |
| 验收门槛 | ~18 | OPTIMIZATION_PLAN §9.5/§10.5（11）+ ADMIN_PLAN §10（6）+ PLAN 验收红线（1） |
| 推送 | 2 | OPTIMIZATION_PLAN §12 |

**Step 3: 逐条标注归属(关键判定)**

对 `/tmp/unchecked.txt` 每一行标注归属,取值只能是:`R0`-`R7` / `§11 冻结` / `§12 推送` / `§14 验收门槛`。

Expected: **0 条"无归属"**。若存在无归属项,回到 Task 3-7 修正文档(不改代码)。

> 去重后的**唯一交付批次 = 7 个(R1-R7)**,且基线中 55 个功能项已收敛至 ≤ 30 条目(新增条目全部属于 R1-R7 的子项)。
...
- [ ] Task 8 归属核对:**0 条"无归属"**,无悬空引用;功能项 ≤ 30 条(基线 55)
