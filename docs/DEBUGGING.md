...
### 19. 历史数据跨用户迁移（单用户 → 多用户）
- **背景**：上线认证系统前是单用户模式（固定 default 用户）；引入注册后，
  需把 default 的历史测试数据（14 会话/2 待办/17 文档）迁移到新注册的 demo 账号
- **排查/设计**：
  1. 确认目标账号 id（避免导入错账号）；
  2. 盘点数据量（含级联子表：消息、分块）；
  3. 判断向量库是否需要迁移——Qdrant payload **只有 document_id 无 user_id**，
     文档 id 不变 → 向量无需任何操作，检索自动跟随新归属
- **解决**：单事务三条 `UPDATE ... SET user_id = <demo> WHERE user_id = <default>`
  （todos / chat_sessions / documents）；消息、附件、分块经**外键级联**自动跟随
- **验证**：数据库行数核对 + 签发目标用户 JWT 走 API 实测（会话/待办/文档全部可见）
...
*共 20 条排障记录 · 覆盖环境/后端/前端/数据四类 · 与 docs/CHANGELOG.md 互为补充*
