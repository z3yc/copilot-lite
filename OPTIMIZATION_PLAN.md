...
**当前基线**：后端 **393 用例 / 覆盖率 83.60%**、ruff 全过；前端 **119 用例**、build 零错误；CLI **15 用例**（沿用 `develop` 基线，R3 未改 CLI）。
（数据为 `fix/citation-jump-target` @ `b0670a7` 实测；R3 起点 `develop` @ `e772c7d` 为后端 380 / 83.39%、前端 107。）
...
- [ ] **合并 `fix/citation-jump-target`**（R3.1 修复：人工核验发现「引用只跳到知识库、到不了具体文件」，含 `resolveCitationTarget` 不降级 + `KbPanel` 按 id 直接打开，两个提交 `14d7aa2..b0670a7`）→ **先合入 `feat/rag-citation-source`**（缺陷属 R3 引入、R3 尚未合并），再随 R3 一并进 `develop`——**待维护者批准**。
  - 验收：前端 119 / 20 文件、`build` 零错误（分支 tip 实测）；后端未改。
  - 回归用例：wiki 缺 `page_id` 必须不可点（`utils/citation.test.ts`）、目标不在列表仍能打开详情（`KbPanel.test.tsx`）。
...
> 本清单为**验收快照**（@ `b0670a7` R3.1 分支 tip 实测；上一快照 @ `af9ed5f` R2 分支 tip），数字随基线推进可能滞后；`develop` 基线见 §1。

- [x] 后端 `pytest` 全绿且覆盖率 ≥ 80%（**393 / 83.60%**）
- [x] 后端 `ruff check .` 全过
- [x] 前端 `npm test` 全绿（**119**）、`npm run build` 零错误
