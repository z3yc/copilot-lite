...
- **帮你做事**：通过可插拔工具集（待办管理、知识库检索；代码沙箱执行、监控通知为预留扩展点）完成实际操作。
...
- 内置工具集：
  ...
  - 💻 **开发类（预留）**：代码仓库索引检索、沙箱内安全执行代码（规划中，未上线）
  - 🖥️ **系统集成（预留）**：服务器状态监控、邮件 / 通知推送（规划中，未上线）
...
- **引用溯源**：每个回答附来源文档与片段位置；
- **权限控制**：文档/会话/待办/记忆按用户隔离（检索链路含 user_id 过滤）；敏感数据脱敏为规划项。
...
| 缓存 | Redis | 预留（登录限流当前为内存实现，多实例/云端切 Redis 滑动窗口） |
...
4. **工具调用的安全边界？**
   → 已实现：工具白名单注册（只执行已注册工具）、参数类型校验与异常兜底、工具层与 REST 层同标准的归属校验、检索层用户隔离。
   → 规划中：代码沙箱（资源限制/网络隔离/超时）与高危操作用户确认机制（human-in-the-loop）。
