...
- **密钥脱敏**（§6.1/§6.8）：MCP `env` 中的 Key、飞书 webhook URL（含 token）**日志一律脱敏**；
- **软删除**（§13）：持仓/调度/通知全 `deleted_at/deleted_by`，列表默认过滤；部分唯一索引 `(user_id, code) WHERE deleted_at IS NULL`；
- **后台任务可开关**（§8）：`SCHEDULER_ENABLED` 测试默认关；ticker 与 MCP 调用均设超时（§17）；
...
| 顺序 | 事项 | 价值 | 依赖/说明 |
|---|---|---|---|
| 0 | **合并待批准分支 + 推送**（§11） | 消除已知 bug/数据风险 | 需批准 |
...
| 10 | 暂缓项（B3/B5/C3/C4/D5/E5） | 按需 | C3 并入 J7 |
