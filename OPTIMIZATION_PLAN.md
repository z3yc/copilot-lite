...
- [ ] **路由回归**：路由判错用例能被测试捕获，关键词兜底仍生效；
- [ ] **白名单生效**：未挂白名单的新工具不会被注入 `tools_agent`；
- [ ] **对照报告产出**：两引擎 token / 延迟 / 准确率三项数据可比；
- [ ] 后端 `pytest` 全绿且覆盖率 ≥80%、`ruff` 全过；前端 `npm test` + `build` 零错误；
- [ ] trajectory 与日志中无密钥。
...
> 本清单为**验收快照**（@ `d2815f6` R2 完成后实测；上一快照 @ `5b5ce2f` J1 合并后），数字随基线推进可能滞后；`develop` 基线见 §1。

- [x] 后端 `pytest` 全绿且覆盖率 ≥ 80%（**370 / 82.81%**）
- [x] 后端 `ruff check .` 全过
- [x] 前端 `npm test` 全绿（**101**）、`npm run build` 零错误
