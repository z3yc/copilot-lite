...
- [ ] **推送 `private-docs`**：含本轮记录（`docs/优化落地记录.md` R2 与 R2.1 节、`docs/plans/` 两份实施计划与预检修订、`面试准备/2026-09-15-流式截断与独白泄漏复盘.md`）——**待维护者确认**（按 §11 只推 GitHub）。
...
| 2 | **R3** | **M5 引用来源标注**：区分 `Wiki/空间/页面` 与 `文档/页码` | ~2 天 | 镜头 1 |
...
> 本清单为**验收快照**（@ `af9ed5f` R2 分支 tip 实测；上一快照 @ `5b5ce2f` J1 合并后），数字随基线推进可能滞后；`develop` 基线见 §1。

- [x] 后端 `pytest` 全绿且覆盖率 ≥ 80%（**370 / 82.81%**）
- [x] 后端 `ruff check .` 全过
- [x] 前端 `npm test` 全绿（**102**）、`npm run build` 零错误
