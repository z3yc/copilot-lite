...
@registry.register
async def todo_list(ctx: ToolContext, status: str | None = None) -> list[dict]:
    """列出待办事项，可按状态过滤（pending/done）。"""
...
async def todo_create(
    ...
    """创建一条待办事项，title 必填，priority 1-5（默认 3），due_date 形如 2025-12-31。"""
    todo = Todo(
        user_id=ctx.user_id,
        title=title,
